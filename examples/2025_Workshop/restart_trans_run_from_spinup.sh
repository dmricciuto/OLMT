...
if [ ${ncfile_init} = /tools/OLMT/examples/2025_Workshop/full_run_US-ARM_ICB1850CNPRDCTCBC.elm.r.0601-01-01-00000.nc]; then
  if [ ! ${site_name} = USARM ] ; then
     # this file in OLMT/examples/Workshop_2025 is specifically for US-ARM site only
     echo " "
     echo "**** EXECUTION HALTED ****"
...
# the following is not needed, because model will setup with option '--nopftdyn' below
